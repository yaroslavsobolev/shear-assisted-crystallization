...
def plot_simulation_shear_dimensionless():
    ...
    data = np.loadtxt('data/rad_200nm.txt', skiprows=1)
...
def plot_relaxation():
    ...
    data = np.loadtxt('data/simulation/max_speed_vs_time.txt', skiprows=49)
...
data = np.loadtxt('data/simulation/max_shear_vs_time_propergammazero.txt', skiprows=233)
...
plt.ylim(-0.1, 2800)
plt.tight_layout()
data2 = np.loadtxt('data/simulation/max_shear_vs_time_nonnewtonian_1.txt', skiprows=1)
