...
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-no-shear/', '4 min-SD.txt')[:, 1]
no_shear_pil.append([4, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-no-shear/', '10 min-SD.txt')[:, 1]
no_shear_pil.append([10, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-no-shear/', '30 min-SD.txt')[:, 1]
no_shear_pil.append([30, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-no-shear/', '60 min-SD.txt')[:, 1]
no_shear_pil.append([60, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-no-shear/', '120 min-SD.txt')[:, 1]
...
do_shear_pil = [[0, 0, 0]]
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-under-shear/Stirring-SD/', '4 min- SD.txt')[:, 1]
do_shear_pil.append([4, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-under-shear/Stirring-SD/', '10 min-SD.txt')[:, 1]
do_shear_pil.append([10, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-under-shear/Stirring-SD/', '30 min-SD.txt')[:, 1]
do_shear_pil.append([30, np.mean(s), np.std(s)])
s = get_sizes_from_file(base_folder+
                    'TA-time-dependence-PIL-under-shear/Stirring-SD/', '60 min-SD.txt')[:, 1]
